...
import {
  ExamplePlugin3PluginSetup,
  ExamplePlugin3PluginStart,
...
export class ExamplePlugin3Plugin
  ...
  public setup(core: CoreSetup): ExamplePlugin3PluginSetup {
    ...
    return this.getCompatibilityResult().then((result) => {
      ...
      core.application.register({
        ...
        navLinkStatus: navStatus === 'Enabled' ? AppNavLinkStatus.visible : AppNavLinkStatus.hidden,
      ...
      return {
        getGreeting() {
          return i18n.translate('XCompat-plugin-example3.greetingText', {
  ...
  async getCompatibilityResult() {
    try {
      ...
      const response = await fetch('/api/example_plugin_3/verify_crosscompatability');
      if (!response.ok) {
  ...
  public start(core: CoreStart): ExamplePlugin3PluginStart {
    return {};
  }
